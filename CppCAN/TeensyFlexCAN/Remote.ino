  ...
  if (sendRPiinterval % 2 == 0)
  {
    ...
    else
    {
      ...
      if (abs(HValue - H_MID) < error && VValue > V_MID + error)
      {
        ...
        motorSpeed = (int)((double)(VValue - V_MID) / (double)(V_HIGH - V_MID) * 0x7fff);

        udp.beginPacket(sendRPiAddress, sendRPiPort);
        udp.printf("%u,%u,%u,%u,", motorSpeed, motorSpeed, 0xffff - motorSpeed, 0xffff - motorSpeed);
        udp.endPacket();
      }
      ...
      if (abs(HValue - H_MID) < error && VValue < V_MID - error)
      {
        ...
        udp.beginPacket(sendRPiAddress, sendRPiPort);
        udp.printf("%u,%u,%u,%u,", 0xffff - motorSpeed, 0xffff - motorSpeed, motorSpeed, motorSpeed);
        udp.endPacket();
      }
      /*
      // 180 degree
