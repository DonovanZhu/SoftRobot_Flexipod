...
double  angle_trot_1                = 120.0;
double  angle_trot_2                = 120.0;

double  freq                       = 0.0;
double  former_base                = 0.6;
double  former_contact_angle       = 120.0;
double  former_angle_trot_1        = 120.0;
double  former_angle_trot_2        = 120.0;
double  former_offset              = 0.5;
double  former_diff                = 0.0;
...
void trot_gait() {
  ...
  if (RPi_comm.diff_speed >= 0.0)
    angle_trot_1 = RPi_comm.contact_angle + fabs(RPi_comm.diff_speed);
  if (RPi_comm.diff_speed <= 0.0)
    angle_trot_2 = RPi_comm.contact_angle + fabs(RPi_comm.diff_speed);
  double k_1_trot = (360.0 - angle_trot_1) / (1.0 - RPi_comm.s_base);
  double k_2_trot = angle_trot_1 / RPi_comm.s_base;
  double b_1_trot = 360.0 - k_1_trot;
  double b_2_trot = (90.0 + 0.5 * angle_trot_1) - k_2_trot * ((90.0 + 0.5 * angle_trot_1) - b_1_trot) / k_1_trot;

  double k_1_trot_2 = (360.0 - angle_trot_2) / (1.0 - RPi_comm.s_base);
  double k_2_trot_2 = angle_trot_2 / RPi_comm.s_base;
  double b_1_trot_2 = 360.0 - k_1_trot_2;
  double b_2_trot_2 = (90.0 + 0.5 * angle_trot_2) - k_2_trot_2 * ((90.0 + 0.5 * angle_trot_2) - b_1_trot_2) / k_1_trot_2;
  
  double T = fabs(1.0 / (RPi_comm.speedcommand / (2 * PI)));

  if (fabs(RPi_comm.speedcommand - freq) > 0.0 || fabs(angle_trot_1 - former_angle_trot_1) > 0.0 || fabs(angle_trot_2 - former_angle_trot_2) > 0.0 || fabs(RPi_comm.contact_angle - former_contact_angle) > 0.0) {
    if (desire_angle[0] >= 0 && desire_angle[0] < 90.0 - angle_trot_1 * 0.5)
      dt = desire_angle[0] * T / k_1_trot;
    else if (desire_angle[0] >= 90.0 - angle_trot_1 * 0.5 && desire_angle[0] < 90.0 + angle_trot_1 * 0.5)
  ...
  former_angle_trot_1 = angle_trot_1;
  former_angle_trot_2 = angle_trot_2;
  former_contact_angle = RPi_comm.contact_angle;
  former_offset = RPi_comm.offset;
  ...
  double dt_2 = dt - RPi_comm.offset * T;
  while (dt_2 > T) {
    dt_2 -= T;
  }
  while (dt_2 < 0) {
  ...
  timestamp_1 = (90.0 - angle_trot_1 * 0.5) / k_1_trot;
  timestamp_2 = (90.0 + angle_trot_1 * 0.5 - b_1_trot) / k_1_trot ;


  if (dt >= 0 && dt < timestamp_1 * T) {
    desire_angle[0] =  k_1_trot / T * dt;
    desire_angle[1] =  desire_angle[0];
  }
  ...
  timestamp_1 = (90.0 - angle_trot_2 * 0.5) / k_1_trot_2;
  timestamp_2 = (90.0 + angle_trot_2 * 0.5 - b_1_trot_2) / k_1_trot_2;

  if (dt_2 >= 0 && dt_2 < timestamp_1 * T) {
    desire_angle[2] = 360.0 - k_1_trot_2 / T * dt_2;
    desire_angle[3] = desire_angle[2];
  }
  else if (dt_2 >= timestamp_1 * T && dt_2 < timestamp_2 * T) {
    desire_angle[2] = 360.0 - (k_2_trot_2 / T * dt_2 + b_2_trot_2);
    desire_angle[3] = desire_angle[2];
  }
  else {
    desire_angle[2] = 360.0 - (k_1_trot_2 / T * dt_2 + b_1_trot_2);
    desire_angle[3] = desire_angle[2];
  }
}

void crawl_gait() {
  double k_1_crawl = (360.0 - RPi_comm.contact_angle) / (1.0 - RPi_comm.s_base);
  double k_2_crawl = RPi_comm.contact_angle / RPi_comm.s_base;
  double b_1_crawl = 360.0 - k_1_crawl;
  double b_2_crawl = (90.0 + 0.5 * RPi_comm.contact_angle) - k_2_crawl * ((90.0 + 0.5 * RPi_comm.contact_angle) - b_1_crawl) / k_1_crawl;
  
  double T = fabs(1.0 / (RPi_comm.speedcommand / (2 * PI)) );

  if (fabs(RPi_comm.speedcommand - freq) > 0.0 || fabs(RPi_comm.s_base - former_base) > 0.0 || fabs(RPi_comm.contact_angle - former_contact_angle) > 0.0) {
    if (desire_angle[0] >= 0 && desire_angle[0] < 90.0 - RPi_comm.contact_angle * 0.5)
      dt = desire_angle[0] * T / k_1_crawl;
    else if (desire_angle[0] >= 90.0 - RPi_comm.contact_angle * 0.5 && desire_angle[0] < 90.0 + RPi_comm.contact_angle * 0.5)
  ...
  timestamp_1 = (90.0 - RPi_comm.contact_angle * 0.5) / k_1_crawl;
  timestamp_2 = (90.0 + RPi_comm.contact_angle * 0.5 - b_1_crawl) / k_1_crawl;
...
void bound_gait() {
  ...
  double k_1_bound = (360.0 - RPi_comm.contact_angle) / (1.0 - RPi_comm.s_base);
  double k_2_bound = RPi_comm.contact_angle / RPi_comm.s_base;
  double b_1_bound = 360.0 - k_1_bound;
  double b_2_bound = (90.0 + 0.5 * RPi_comm.contact_angle) - k_2_bound * ((90.0 + 0.5 * RPi_comm.contact_angle) - b_1_bound) / k_1_bound;
  double T = fabs(1.0 / (RPi_comm.speedcommand / (2 * PI)));
  ...
  timestamp_1 = (90.0 - RPi_comm.contact_angle * 0.5) / k_1_bound;
  timestamp_2 = (90.0 + RPi_comm.contact_angle * 0.5 - b_1_bound) / k_1_bound;
...
void flip_gait() {
  double front_leg_step = 4.0;
  double back_leg_step = 4.2;

  if (flip_states == 0) {
    if (fabs(120.0 - desire_angle[0]) < 5.0)
      desire_angle[0] = 120.0;
    else
      desire_angle[0] += front_leg_step;
  
    if (fabs(240.0 - desire_angle[3]) < 5.0)
      desire_angle[3] = 240.0;
    else
      desire_angle[3] -= front_leg_step;

    if (fabs(160.0 - desire_angle[1]) < 5.0)
      desire_angle[1] = 160.0;
    else
    {
      if (shaft_angle_meas[0] < 180.0 && shaft_angle_meas[0] > 30.0)
        desire_angle[1] += back_leg_step;
    }
    if (fabs(200.0 - desire_angle[2]) < 5.0)
      desire_angle[2] = 200.0;
    else
    {
      if (shaft_angle_meas[0] < 180.0 && shaft_angle_meas[0] > 30.0)
        desire_angle[2] -= back_leg_step;
    }
    if (fabs(160.0 - shaft_angle_meas[1]) < 5.0)
      flip_states = 1;
  }
  else if (flip_states == 1) {
    if (fabs(60.0 - desire_angle[0]) <= 5.0)
      desire_angle[0] = 60.0;
    else
      desire_angle[0] -= front_leg_step;
  
    if (fabs(300.0 - desire_angle[3]) < 5.0)
      desire_angle[3] = 300.0;
    else
      desire_angle[3] += front_leg_step;

    if (fabs(120.0 - desire_angle[1]) <= 5.0)
      desire_angle[1] = 120.0;
    else
      desire_angle[1] -= back_leg_step;
  
    if (fabs(240.0 - desire_angle[2]) < 5.0)
      desire_angle[2] = 240.0;
    else
      desire_angle[2] += front_leg_step;
  }

}

void flip_gait_180() {
  double front_leg_step = 2.0;
  double back_leg_step = 4.0;
  if (fabs(270.0 - desire_angle[1]) < 5.0)
    desire_angle[1] = 270.0;
  else
    desire_angle[1] += back_leg_step;

  if (fabs(90.0 - desire_angle[2]) < 5.0)
    desire_angle[2] = 90.0;
  else
    desire_angle[2] -= back_leg_step;

  if (fabs(270.0 - desire_angle[0]) < 5.0)
    desire_angle[0] = 270.0;
  else
    desire_angle[0] += front_leg_step;

  if (fabs(90.0 - desire_angle[3]) < 5.0)
    desire_angle[3] = 90.0;
  else
    desire_angle[3] -= front_leg_step;
}
...
void legs_sync() {
  double reset_step = 0.18;
  flip_states = 0;
  desire_angle[0] = command_angle[0];
  if (fabs(300.0 - desire_angle[0]) < 1.0) {
    desire_angle[0] = 300.0;
  }
  else {
      desire_angle[0] -= reset_step;
  }

  desire_angle[1] = command_angle[1];
  if (fabs(300.0 - desire_angle[1]) < 1.0) {
    desire_angle[1] = 300.0;
  }
  else {
      desire_angle[1] += reset_step;
  }

  desire_angle[2] = command_angle[2];
  if (fabs(60.0 - desire_angle[2]) < 1.0) {
    desire_angle[2] = 60.0;
  }
  else {
      desire_angle[2] -= reset_step;
  }

  desire_angle[3] = command_angle[3];
  if (fabs(60.0 - desire_angle[3]) < 1.0) {
    desire_angle[3] = 60.0;
  }
  else {
      desire_angle[3] += reset_step;
  }
}
...
void loop() {
  ...
  if (RPi_comm.speedcommand != 0.0) {
    if (RPi_comm.gait != former_gait && RPi_comm.gait != 4 && RPi_comm.gait != 5 && RPi_comm.gait != 8 && RPi_comm.gait != 9 && RPi_comm.gait != 10) {
      ...
      if (former_gait == 1)
        ...
        legs_sync();
      else if (former_gait == 9)
        rotation_sync();
      else if (former_gait == 10)
        flip_gait_180();
      if (fabs(eul.roll_e) > PI / 2.0 && RPi_comm.gait != 5 && RPi_comm.gait != 6 && RPi_comm.gait != 7 && RPi_comm.gait != 8 && RPi_comm.gait != 9) {
        for (int i = 0; i < NB_ESC; ++i) {
    ...
    if (!gait_transfer) {
      if (RPi_comm.gait == 1)
        ...
        legs_sync();
      else if (RPi_comm.gait == 9)
        rotation_sync();
      else if (RPi_comm.gait == 10)
        flip_gait_180();
    }
    ...
    if (fabs(eul.roll_e) > PI / 2.0 && RPi_comm.gait != 4 && RPi_comm.gait != 5 && RPi_comm.gait != 6 && RPi_comm.gait != 7 && RPi_comm.gait != 8 && RPi_comm.gait != 9 && RPi_comm.gait != 10) {
      for (int i = 0; i < NB_ESC; ++i) {
