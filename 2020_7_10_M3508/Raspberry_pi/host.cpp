...
double 	desire_speed[2] = {0.0, 0.0};		// The deisred speed

// Receiving the desired speed data by UDP and hold it in this class
class MotorSpeed {
public:
	double 	robot_command[2];
...
int Host_init_port(uint32_t serial_nb) {
	...
	RPi_comm.speedcommand[0] =  0.0;
	RPi_comm.speedcommand[1] =  0.0;
...
int Host_comm_update( uint32_t            serial_nb,
	...
	RPi_comm.speedcommand[0] = desire_speed[0];
	RPi_comm.speedcommand[1] = desire_speed[1];
...
int main(int argc, char *argv[]) {
	...
	/******************************************************************/
		...
		while (1) {
			...
			recv.clear();
			obj.convert(recv);
			command_recv[0] = int((recv[0].robot_command[0] / 500.0) * 100000.0);
			command_recv[1] = int((recv[0].robot_command[1] / 500.0) * 100000.0);
	...
	/******************************************************************/
		...
		int x = 0;
		while(1) {
			desire_speed[0] = (double) *shmem / 100000.0 * 500.0;
			desire_speed[1] = (double) *(shmem + 1) / 100000.0 * 500.0;
