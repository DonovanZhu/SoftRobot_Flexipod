...
// byte message[26];
byte message[16];
const int16_t MESSAGE_START_TOKEN = 0x8000;
int issend = 0;
...
void loop()
{
  ...
  if (time_delta >= 2500)
  {
    ...
    for (uint8_t i = 0; i < 4; i++)
    {
      ...
      message[i * 2] = highByte(d_position_measured[i]);
      message[i * 2 + 1] = lowByte(d_position_measured[i]);
      
      message[i * 2 + 8] = highByte(d_speed_measured[i]);
      message[i * 2 + 9] = lowByte(d_speed_measured[i]);
