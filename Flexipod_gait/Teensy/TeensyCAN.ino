  ...
  if (RPi_comm.speedcommand != 0.0) {
    ...
    if (fabs(RPi_comm.speedcommand - freq) > 0.01) {
      if (RPi_comm.speedcommand > 0.0) {
        ...
        if (desire_angle[0] >= 0 && desire_angle[0] < 45.0)
          dt = T - desire_angle[0] * T / 675.0;
        else if (desire_angle[0] >= 45.0 && desire_angle[0] < 135.0)
          dt = T - (desire_angle[0] - 115.0 / 3.0) * T / 150.0;
        else
          dt = T - (desire_angle[0] + 315.0) * T / 675.0;
      }
    }
    ...
    if (RPi_comm.speedcommand > 0.0) {
      ...
      if (dt >= 14.0 / 15.0 * T) {
        desire_angle[0] =  675.0 / T * (T - dt);
        desire_angle[2] =  360.0 - desire_angle[0];
      }
      else if (dt < 14.0 / 15.0 * T && dt >= 1.0 / 3.0 * T) {
        desire_angle[0] =  150.0 / T * (T - dt) + 115.0 / 3.0;
        desire_angle[2] =  360.0 - desire_angle[0];
      }
      else {
        desire_angle[0] = 675.0 / T * (T - dt) - 315.0;
        desire_angle[2] = 360.0 - desire_angle[0];
      }

      if (dt_2 >= 14.0 / 15.0 * T) {
        desire_angle[1] =  675.0 / T * (T - dt_2);
        desire_angle[3] =  360.0 - desire_angle[1];
      }
      else if (dt_2 < 14.0 / 15.0 * T && dt_2 >= 1.0 / 3.0 * T) {
        desire_angle[1] =  150.0 / T * (T - dt_2) + 115.0 / 3.0;
        desire_angle[3] =  360.0 - desire_angle[1];
      }
      else {
        desire_angle[1] = 675.0 / T * (T - dt_2) - 315.0;
        desire_angle[3] = 360.0 - desire_angle[1];
      }
    }
  }
