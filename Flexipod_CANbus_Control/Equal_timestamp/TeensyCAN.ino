...
// 256 and 16. Check: https://github.com/tonton81/FlexCAN_T4
//
...
void Teensy_comm_update(void) {
  ...
  if (in_cnt == (int)sizeof(RPi_comm)) {
    ...
    for (int i = 0; i < NB_ESC; i++) {
      ...
      Teensy_comm.torque[i] = (double)(t2 - t1) / 1000000.0;
    }
    t1 = t2;
...
void Control_loop() {
  ...
  for (int i = 0; i < NB_ESC; ++i) {
    ...
    if (abs(error[i]) < 70.0)
      ...
      speed_command[i] += 14.0 * (error[i] - error_former[i]);
...
void CAN_recv_data(const CAN_message_t &msg) {
  ...
  if (!can_received[msg.id - 0x201]) {
    ...
    rpm = 0;
    rpm |= (int16_t)(unsigned char)msg.buf[2] << 8;
    rpm |= (int16_t)(unsigned char)msg.buf[3];
    ...
    if (rpm < DirectionBoundary)
      speed_meas[msg.id - 0x201] = (double)rpm / DRIVE_RATIO;
    else
      speed_meas[msg.id - 0x201] = - (double)(maxBoundary - rpm) / DRIVE_RATIO;
        
    angle = 0;
    angle |= (int16_t)(unsigned char)msg.buf[0] << 8;
    angle |= (int16_t)(unsigned char)msg.buf[1];

    angle_calculate((double)angle / 8191.0 * 360.0, msg.id - 0x201);
        
    torque = 0;
    torque |= (int16_t)(unsigned char)msg.buf[4] << 8;
    torque |= (int16_t)(unsigned char)msg.buf[5];
    torque_meas[msg.id - 0x201] = (double)torque;
        
    can_received[msg.id - 0x201] = true;
    recv_flag++;
  }  
}

void setup() {
  // Open Serial port in speed 4000000 Baudrate
  ...
  CAN.setMB(MB1,RX,STD);
  CAN.setMB(MB2,RX,STD);
  CAN.setMB(MB3,RX,STD);
  CAN.setMB(MB4,TX,STD);

  CAN.enableMBInterrupts();
  CAN.onReceive(MB0,CAN_recv_data);
  CAN.onReceive(MB1,CAN_recv_data);
  CAN.onReceive(MB2,CAN_recv_data);
  CAN.onReceive(MB3,CAN_recv_data);
  CAN.setMBFilter(MB0,0x201);
  CAN.setMBFilter(MB1,0x202);
  CAN.setMBFilter(MB2,0x203);
  CAN.setMBFilter(MB3,0x204);
  //CAN_init();
...
void loop() {
  ...
  if (in_cnt == (int)sizeof(RPi_comm)) {
    ...
    // Read data from motors
    Control_loop();
    CAN.write(MB4, msg_send); // Write the message on CAN bus
    //delay(1);
