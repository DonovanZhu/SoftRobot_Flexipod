...
public:
	...
	int		gait_type;
	MSGPACK_DEFINE(robot_command, gait_type);
};
...
int main( ) {
	...
	/******************************************************************/
		...
		while (1) {
			...
			command_recv[0] = (int)(recv[0].robot_command * 1000000000.0);
			
			command_recv[1] = recv[0].gait_type;
			memcpy(shmem, command_recv, sizeof(command_recv));
			printf("%f\t%d\n", recv[0].robot_command, recv[0].gait_type);
		}
	} 
	...
	/******************************************************************/
		...
		while(1) {
			...
			desire_speed = (double) *shmem / 1000000000.0 ;
			gait_type = *(shmem + 1);
