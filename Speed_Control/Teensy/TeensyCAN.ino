...
double  freq[NB_ESC]               = {0.0, 0.0, 0.0, 0.0};
...
void speed_control() {
  ...
  for (int i = 0; i < NB_ESC; i++) {
    if (RPi_comm.speedcommand[i] != 0.0) {
      T[i] = fabs(360.0 / (RPi_comm.speedcommand[i] * 180.0 / PI));
      if (fabs(RPi_comm.speedcommand[i] - freq[i]) > 0.0) {
        dt[i] = desire_angle[i] / fabs((RPi_comm.speedcommand[i] * 180.0 / PI));
      }
      ...
      freq[i] = RPi_comm.speedcommand[i];
      while (dt[i] > T[i]) {
        dt[i] -= T[i];
      }
      while (dt[i] < 0) {
      ...
      desire_angle[i] = (fabs(RPi_comm.speedcommand[i]) * 180.0 / PI) * dt[i];
    }
...
void loop() {
  ...
  if (in_cnt == (int)sizeof(RPi_comm)) {
    ...
    for (int i = 0; i < NB_ESC; i++) {
      ...
      Teensy_comm.rspeed[i] = desire_angle[i];
      Teensy_comm.torque[i] = torque_meas[i];
      Teensy_comm.motor_command[i] = RPi_comm.speedcommand[i];
    }
